        ...
        <a target="_blank" :href="item['itemLink']">
          <el-card shadow="hover" style="text-align:left" :body-style="{padding:'10px'}">
            <div class="rankinglist-music">
              ...
              <el-image style="width:90px;margin-right:20px;flex-shrink:0" :src="item['itemImg']" fit="scale-down"></el-image>
              <div style="width:60%">
                <p class="song-name">{{ item["itemTitle"] }}</p>
                <el-link :underline="false" type="primary" target="_blank" :href="item['itemSingerLink']">
...
<script>
  export default {
    ...
    created () {
      this.axios.get("/musicList").then(response => {
