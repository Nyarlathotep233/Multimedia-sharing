var SkyRTC = function () {
  ...
  skyrtc.prototype.createDataChannel = function (socketId, label) {
    ...
    try {
      channel = pc.createDataChannel(label); //新建dataChannel,pc是个PeerConnection实例
    } catch (error) {
  ...
  skyrtc.prototype.sendFileAccept = function (sendId) {
    var that = this,
      ...
      packet;
    if (!channel) {
      that.emit("receive_file_error", new Error("Channel has been destoried"), sendId, fileInfo.socketId);
    }
    packet = {
      type: "__file",
      signal: "accept",
