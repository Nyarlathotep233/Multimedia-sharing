<template>
  <div>
    ...
    <el-row type="flex" justify="center">
      <el-col :span="11">
        <el-input v-model="search" placeholder="请输入内容" type="primary" @keyup.enter.native="searchMusic"></el-input>
      </el-col>
    ...
    <el-row type="flex" justify="center">
      <el-col :span="16">
        <el-row type="flex" justify="center">
          <el-col>
            <div class="whiteblock" :style="'padding: 10px;min-height:' + minHeight + 'px;'" v-loading="loading" element-loading-text="搜索中">
                ...
                <el-table :data="miguSearchResult" style="width: 100%">
                  <el-table-column prop="musicTitle" label="歌曲" width="340">
                    <template slot-scope="scope">
                      <a :href="scope.row.musicLink" target="_blank" class="music-search-result-link">{{ scope.row.musicTitle }}</a>
                    </template>
                  </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      searchMusic () {
        if (!this.search == "") {
          ...
          this.minHeight = 100;
          this.axios
            .get("/searchMusic?searchContent=" + this.search)
            .then(response => {
              this.first = false;
              this.loading = false;
              this.minHeight = 0;
    ...
    created () {
      this.axios.get("/musicNewAlbum").then(response => {
      ...
      this.axios.get("/musicNewSong").then(response => {
      ...
      this.axios.get("/musicRecommend").then(response => {
